                ...
                await message.channel.send(`${opponent}, düello isteği geldi. Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.)`);
                        ...
                        while (userHP > 0 && oppoHP > 0) {
                                ...
                                if (choice === 'saldır') {
                                        ...
                                        await message.channel.send(`${user}, Su kütlesi saldırısı ile **${damage}** hasar vurdu!`);
                                        dealDamage(damage);
                                        reset();
                                } else if (choice === 'savun') {
                                        await message.channel.send(`${user}, kendisini Protector Kalkanı ile savundu!`);
                                        guard = true;
                                        reset(false);
                                        ...
                                        if (!miss) {
                                                ...
                                                await message.channel.send(`${user},  yardım aldın sonra elektirk saldırısı ile  **${damage}** hasar vurdun!!`);
                                                dealDamage(damage);
                                        } else {
                                                await message.channel.send(`${user},  elektrik saldırısı henüz dolmadığı için elektrik saldırısı atamadı`);
                                        }
                                        reset();
                                } else if (choice === 'kaç') {
